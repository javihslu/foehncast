...
def compute_quality_index(
    ...
    """Compute a 0-5 quality index per row, skipping rows with missing values."""
    _require_columns(features_df)

    complete = features_df.dropna(subset=sorted(_REQUIRED_COLUMNS))
    labeling_cfg = get_labeling_config()
    minimum_wind = _minimum_rideable_wind(labeling_cfg, rider_config)
    quality = complete.apply(_score_row, axis=1, args=(labeling_cfg, minimum_wind))
...
def label_dataset(
    ...
    """Label the feature dataframe, keeping only rows that could be scored."""
    quality = compute_quality_index(features_df, rider_config)
    labeled = features_df.loc[quality.index].copy()
    labeled["quality_index"] = quality
    return labeled
