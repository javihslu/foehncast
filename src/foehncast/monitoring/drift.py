...
def push_drift_metrics(report: DriftReport) -> None:
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as client:
            for line in _statsd_lines(report, prefix):
                client.sendto(line.encode("utf-8"), (host, port))
    except OSError:
...
def _drift_report_path(
    report_kind: str, dataset_name: str, dataset_version: str
) -> ReportLocation:
    # Key by version too so different comparisons do not overwrite each other.
    return report_object_path(
        _drift_report_dir(),
        f"drift-{report_kind}-{dataset_name}-{dataset_version}-latest.json",
    )
...
def _persist_drift_report(report: DriftReport) -> None:
    ...
    path = _drift_report_path(
        report.report_kind, report.dataset_name, report.dataset_version
    )
...
def read_drift_report(
    ...
    path = _drift_report_path(report_kind, dataset_name, dataset_version)
    try:
...
def _run_evidently_data_drift(
    ...
    # Constant columns make Evidently's internal correlations divide by zero;
    # the resulting NaN is handled, so keep the warning out of the job logs.
...
def _evidently_classes() -> tuple[Any, Any]:
    try:
        # Evidently 0.7 emits SyntaxWarnings on import under Python 3.12.
