...
mean_predicted_quality = Gauge(
    "foehncast_inference_mean_predicted_quality",
    "Mean predicted quality index (0-5 scale) of the latest forecast per spot",
    ["spot_id"],
    registry=_registry,
)


def observe_mean_predicted_quality(spot_id: str, value: float) -> None:
    """Record the mean predicted quality for a spot (0-5 scale)."""
    bounded = max(0.0, min(5.0, float(value)))
    mean_predicted_quality.labels(spot_id).set(bounded)
