...
def main() -> None:
    ...
    with rider_tab:
        # Heavy load deferred to the rider tab so sidebar renders instantly.
        ...
        if dashboard_error is not None:
            ...
            prewarm_spot_caches(
                all_spot_ids,
                json.dumps(dashboard_data.get("predictions", []), default=str),
            )
            # Show profile card in sidebar now that data is available.
            with st.sidebar:
                st.markdown(
                ...
                st.caption(
                    "Drive-time ranking uses the rider home from config.yaml "
