...
@st.cache_data(ttl=1800, show_spinner=False)
def _spot_quality_timeline(spot_id: str, predictions_json: str) -> pd.DataFrame:
    """Build a quality-index timeline combining past predictions, actuals, and future forecast.

    Returns a DataFrame with columns: time, quality_index, series
    where series is one of: "Predicted (past)", "Observed", "Forecast"

    predictions_json: JSON-serialized list of prediction dicts (for cacheability).
    """
    ...
    # 1. Past predictions from the durable prediction log for this spot.
    try:
    ...
    # 2. Observed actuals: fetch archive weather and compute quality for the same window.
    try:
        spot = next((s for s in get_spots() if s["id"] == spot_id), None)
        if spot is not None:
            # Use 2 days ago as a safe archive window (Open-Meteo has ~5 day latency
            # for full archive, but recent analysis data is usually available).
...
def _render_rider_console(
    ...
    if focus_spot_id is not None:
        # --- Quality index timeline (model predictions vs actuals) --------
        predictions_list = dashboard_data.get("predictions", [])
        quality_frame = _spot_quality_timeline(
            focus_spot_id, json.dumps(predictions_list, default=str)
        )
        if not quality_frame.empty:
            st.subheader(f"Ride quality — {_spot_label(spot_lookup, focus_spot_id)}")
            ...
            # Rideable threshold at quality >= 3.
            q_threshold = (
        ...
        # --- Wind speed timeline (raw weather context) --------------------
        timeline_frame = _focus_spot_timeline(focus_spot_id)
        with st.expander(
            f"Wind speed — {_spot_label(spot_lookup, focus_spot_id)}", expanded=False
        ):
...
def _render_log_entries(logs: list[dict[str, str]]) -> None:
    """Render pre-fetched log entries (same display as _render_log_panel)."""
    if not logs:
        st.markdown(
...
def _render_pipeline_rail(rail: dict[str, Any], prefetched: dict[str, Any]) -> None:
    ...
    *prefetched* contains ``success``, ``summary_ts``, ``chip_values``,
    ``stages``, and ``logs`` already resolved by the batched fan-out.
    """
...
def _render_drift_breakdown() -> None:
    ...
    Shows average drift score per spot and top drifted features,
    giving operators immediate visibility into data quality.
    """
    st.markdown(
        '<div style="font-family:Manrope,sans-serif;font-weight:800;'
        'font-size:0.95rem;color:#07252a;letter-spacing:0.01em;padding-bottom:8px">'
    ...
    # Fetch per-spot avg drift scores and per-column drift counts in parallel.
    spot_query = (
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        spot_future = pool.submit(_prom_query_vector, spot_query)
        column_future = pool.submit(_prom_query_vector, column_query)
    ...
    if spot_results:
        ...
        for entry in spots_sorted:
            ...
            score = entry["value"]
            # Color: green < 0.3, amber 0.3-0.7, red > 0.7
            if score < 0.3:
                bar_color = "#0e8a86"
            elif score < 0.7:
                bar_color = "#ff7a26"
    ...
    # Top drifted columns (features with drift_detected sum > 0)
    if column_results:
...
def _render_pipelines_panel() -> None:
    ...
    # Also pre-fetch vector queries (stage state/duration) and logs in parallel.
    # This eliminates the remaining sequential calls inside each rail.
    ...
    def _fetch_logs(job_key: str) -> list[dict[str, str]]:
        return _list_job_logs(_PIPELINE_JOB_NAMES[job_key], limit=6)

    with ThreadPoolExecutor(max_workers=12) as pool:
        # Scalar batch
        scalar_future = pool.submit(_prom_query_batch, all_scalar_exprs)
        # Vector queries per rail (only if the rail has stages)
        stage_futures = {
            idx: pool.submit(_fetch_stages, rail)
            for idx, rail in enumerate(_PIPELINE_RAILS)
            if rail["stage_order"]
        }
        # Log fetches per rail
        log_futures = {
