...
def _payload(model_version: str, *points: tuple[str, float]) -> dict[str, object]:
    """Build a single-spot silvaplana prediction payload from (time, quality) points."""
    return {
        ...
        "predictions": [
            {
                ...
                "forecast": [
                    {"time": time, "quality_index": quality_index}
                    for time, quality_index in points
                ],
            }
        ],
    }
...
def test_append_prediction_log_and_read_prediction_log_round_trip(
    ...
    payload = _payload(
        "7",
        ("2025-01-01T00:00:00+00:00", 2.4),
        ("2025-01-01T01:00:00+00:00", 2.8),
    )

    logged_at = _NOW - timedelta(days=1) + timedelta(hours=10)
    ...
    assert frame["prediction_timestamp"].iloc[0].isoformat() == logged_at.isoformat()
...
def test_read_prediction_history_prefers_durable_event_store_when_available(
    ...
    prediction_log.append_prediction_log(
        _payload("7", ("2025-01-01T00:00:00+00:00", 2.1)),
        endpoint="predict",
        path=remote_log_path,
        event_path=shared_event_path,
        logged_at=_NOW - timedelta(days=1) + timedelta(hours=9),
    )
    prediction_log.append_prediction_log(
        _payload("7", ("2025-01-01T01:00:00+00:00", 2.3)),
        endpoint="predict",
        path=local_log_path,
        event_path=shared_event_path,
        logged_at=_NOW - timedelta(days=1) + timedelta(hours=10),
...
def test_append_prediction_log_rejects_event_path_matching_working_log(
    ...
    with pytest.raises(
        ...
        prediction_log.append_prediction_log(
            _payload("7", ("2025-01-01T00:00:00+00:00", 2.4)),
            endpoint="predict",
            path=log_path,
            event_path=log_path,
            logged_at=_NOW - timedelta(days=1) + timedelta(hours=10),
...
def test_append_prediction_log_trims_to_recent_rows(
    ...
    log_path = tmp_path / "prediction-log.jsonl"
    monkeypatch.setenv("FOEHNCAST_PREDICTION_LOG_MAX_ROWS", "3")

    prediction_log.append_prediction_log(
        _payload(
            "7",
            ("2025-01-01T00:00:00+00:00", 2.1),
            ("2025-01-01T01:00:00+00:00", 2.2),
        ),
        endpoint="predict",
        path=log_path,
        max_rows=3,
    ...
    prediction_log.append_prediction_log(
        _payload(
            "7",
            ("2025-01-01T02:00:00+00:00", 2.3),
            ("2025-01-01T03:00:00+00:00", 2.4),
        ),
        endpoint="predict",
        path=log_path,
        max_rows=3,
        logged_at=_NOW - timedelta(days=1) + timedelta(hours=11),
    )

    frame = prediction_log.read_prediction_log(log_path, max_rows=3)
    env_frame = prediction_log.read_prediction_log(log_path)
    ...
    assert list(env_frame["quality_index"]) == [2.2, 2.3, 2.4]
...
def test_append_prediction_log_preserves_recent_rows_per_model_version(
    ...
    def append(model_version: str, quality_index: float, hour: int) -> None:
        prediction_log.append_prediction_log(
            _payload(
                model_version,
                (f"2025-01-01T{hour:02d}:00:00+00:00", quality_index),
            ),
            endpoint="predict",
            path=log_path,
            max_rows=2,
            logged_at=_NOW - timedelta(days=1) + timedelta(hours=10, minutes=hour),
        )
...
def test_append_prediction_log_prunes_stale_model_rows_outside_retention_window(
    ...
    prediction_log.append_prediction_log(
        _payload("6", ("2025-01-01T00:00:00+00:00", 1.5)),
        endpoint="predict",
        path=log_path,
        max_rows=2,
        retention_days=30,
        logged_at=_NOW - timedelta(days=46),
    )
    prediction_log.append_prediction_log(
        _payload(
            "7",
            ("2025-01-01T01:00:00+00:00", 2.4),
            ("2025-01-01T02:00:00+00:00", 2.6),
        ),
        endpoint="predict",
        path=log_path,
        max_rows=2,
        retention_days=30,
        logged_at=_NOW - timedelta(days=1),
    )
...
def test_read_prediction_log_uses_minimum_two_window_retention_from_config(
    ...
    prediction_log.append_prediction_log(
        ...
        path=log_path,
        max_rows=2,
        logged_at=_NOW - timedelta(days=21),
    )
    prediction_log.append_prediction_log(
        _payload("7", ("2025-01-01T01:00:00+00:00", 2.3)),
        endpoint="predict",
...
def test_emit_prediction_drift_metrics_filters_to_current_model_version(
    ...
    prediction_log.append_prediction_log(
        ...
        endpoint="predict",
        path=log_path,
        logged_at=_NOW - timedelta(days=2) + timedelta(hours=10),
    )
...
def test_emit_prediction_drift_metrics_uses_recent_rows_for_current_model_version(
    ...
    report = prediction_log.emit_prediction_drift_metrics(
        _payload("7", ("2025-01-01T03:00:00+00:00", 2.2)),
        endpoint="predict",
        path=log_path,
        max_rows=2,
    )
...
def test_emit_prediction_drift_metrics_prefers_durable_event_history_when_available(
    ...
    report = prediction_log.emit_prediction_drift_metrics(
        _payload("7", ("2025-01-01T01:00:00+00:00", 2.4)),
        endpoint="predict",
        path=local_log_path,
        event_path=shared_event_path,
        max_rows=10,
