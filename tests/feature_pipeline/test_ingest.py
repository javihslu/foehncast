...
class TestHourlyToDataframe:
    ...
    def test_accepts_undefined_unit_with_warning(self, caplog) -> None:
        """Open-Meteo reports 'undefined' for higher-altitude wind fields."""
        response = {
            "hourly": {
                **MOCK_HOURLY_RESPONSE["hourly"],
                "wind_speed_80m": [10.0, 11.0, 12.0],
            },
            "hourly_units": {
                **MOCK_HOURLY_RESPONSE["hourly_units"],
        ...
        df = _hourly_to_dataframe(response, timezone="Europe/Zurich")
        assert "wind_speed_80m" in df.columns
        assert df.attrs["hourly_units"]["wind_speed_80m"] == "km/h"
