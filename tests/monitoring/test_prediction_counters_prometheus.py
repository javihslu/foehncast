...
from foehncast.monitoring import prediction_counters_prometheus
from tests.prometheus_assertions import metric_value


@pytest.fixture(autouse=True)
def reset_prediction_monitoring_state() -> None:
    prediction_counters_prometheus._reset_prediction_monitoring_state()
    yield
    prediction_counters_prometheus._reset_prediction_monitoring_state()


def test_render_prediction_counters_prometheus_metrics_uses_endpoint_and_result_labels() -> (
    None
):
    prediction_counters_prometheus.record_prediction_monitoring_schedule(
        "predict",
        "scheduled",
    ...
    prediction_counters_prometheus.record_prediction_monitoring_execution(
        "predict",
        "succeeded",
    ...
    payload = prediction_counters_prometheus.render_prediction_counters_prometheus_metrics().decode(
        "utf-8"
    )
...
def test_render_prediction_counters_prometheus_metrics_handles_empty_state() -> None:
    payload = prediction_counters_prometheus.render_prediction_counters_prometheus_metrics().decode(
        "utf-8"
    )
...
def test_ephemeral_metric_prefix_constant_covers_all_rendered_metric_names() -> None:
    ...
    prefix = prediction_counters_prometheus.EPHEMERAL_METRIC_PREFIX
