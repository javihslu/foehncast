...
def test_validate_feature_pipeline_context_serializes_timestamp_range_violations(
    ...
    feature_context = _orch_feature._feature_pipeline_context(run_key="timestamp-test")
    feature_context.engineered_spots = ["silvaplana"]
    run_dir = feature_context.run_dir
    _orch_feature._write_feature_pipeline_frame(
        _orch_feature._feature_pipeline_stage_path(run_dir, "feature", "silvaplana"),
        feature_df,
    )
    ...
    validation_path = _orch_feature._feature_pipeline_validation_path(
        run_dir, "silvaplana"
    )
...
def test_run_feature_pipeline_job_context_reports_drift_and_logs_mlflow(
    ...
    assert logged["metrics"] == {
        "expected_spot_count": 2.0,
        "fetched_spot_count": 2.0,
        "engineered_spot_count": 2.0,
        "validated_spot_count": 2.0,
        "stored_spot_count": 2,
        "drifted_spot_count": 1,
...
def test_training_run_metrics_and_params_normalize_mlflow_run_data(
    ...
    metrics, params = _orch_training._training_run_metrics_and_params("run-123")
...
def test_run_feature_drift_detection_step_compares_reference_dataset(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(_orch_drift, "get_spots", lambda: [{"id": "bodensee"}])

    slices = {
        ("bodensee", "train"): pd.DataFrame({"wind_speed_10m": [1.0, 2.0, 3.0]}),
        ("bodensee", "forecast"): pd.DataFrame({"wind_speed_10m": [8.0, 9.0]}),
    }
    monkeypatch.setattr(
        _orch_drift,
        "_read_optional_feature_slice",
        lambda spot_id, dataset: slices.get((spot_id, dataset), pd.DataFrame()),
    )

    emit_calls: list[dict[str, object]] = []

    def fake_emit(*, spot_id, dataset, reference_df, current_df) -> bool:
        emit_calls.append(
            {
                "dataset": dataset,
                "ref_rows": len(reference_df),
    ...
    result = orchestration.run_feature_drift_detection_step(
        dataset="forecast", reference_dataset="train"
    )

    assert result["comparison"] == "train-vs-forecast"
    assert result["drifted_spots"] == ["bodensee"]
    assert emit_calls == [
        {"dataset": "train-vs-forecast", "ref_rows": 3, "cur_rows": 2}
    ]
