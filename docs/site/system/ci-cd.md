...
<div class="mermaid">
flowchart TD
    classDef trigger fill:#fff4e6,stroke:#c2410c
    classDef gha fill:#e6f4f1,stroke:#0f766e
    classDef gcp fill:#0f2530,stroke:#0f766e,color:#fff
    classDef outcome fill:#eef2f7,stroke:#475569
...
sequenceDiagram
    ...
    GH->>GH: Deploy Docs
    GH->>CB: Webhook (Developer Connect)
    CB->>CB: Trigger fires (path match)
    CB->>AR: Build and push image
    CB->>CR: Deploy new revision (image from AR)
