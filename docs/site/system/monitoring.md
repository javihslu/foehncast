...
The feature pipeline records the timestamp of each Feast materialization on its run summary, and `foehncast_feast_max_feature_age_seconds{dataset}` renders the age of the newest materialization at scrape time (now minus that timestamp). Because the pipeline is scheduled every six hours, the `FeatureDataStale` alert fires when the age for any dataset exceeds eight hours (the schedule plus a two-hour grace) for 15 minutes, flagging a stalled feature refresh. Older summaries without a recorded timestamp simply render no gauge, so the signal is backward compatible.
...
| `FeatureDataStale` | Feast feature age > 8h for 15 min |
